<template>
	<div class="visualizing-demo2">
		...
		<div class="big-data-up">
			...
			<div class="up-center">
				<span>宁夏莱芝科技智慧农业系统平台</span>
			</div>
			<div class="up-right">
				<el-dropdown size="small">
					<span class="el-dropdown-link">
						{{ state.dropdownActive }}
						<SvgIcon name="ele-ArrowDown" class="el-icon--right" /> </span
					>getTableData
					<template #dropdown>
						<el-dropdown-menu>
				...
				<div class="flex-warp-item">
					<div class="flex-warp-item-box">
						...
						<div class="flex-content flex-content-overflow">
							...
							<div class="sky-dd">
								<div class="sky-dl" v-for="(item,index) in state.tableData.data" :key="index">
									<div>{{item.imei}}</div>
									...
									<div>{{item.wdir}}</div>
									<div>{{item.wdirzStr}}</div>
									<div>{{item.airPressure}}</div>
									<div>{{item.hv}}</div>
									<div>{{item.currentRainfall}}</div>
									<div>{{item.soilTerm}}</div>
									<div>{{item.soilHum}}</div>
									<div>{{item.conductivity}}</div>
...
import dataApi from '/@/api/demo/demo2';
...
const getTableData = async () => {
	...
	const res = await dataApi.useDataApi();
	const data1 = [];
